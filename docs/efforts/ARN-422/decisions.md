...
- Removed the now-obsolete `genesis_bootstrap_runtime_recovery_allows_skip` fn + its test (the probe no longer maps recovery outcomes). 36/37 startup tests green, fmt/clippy clean.
...
- **Where:** crates/temperpaw/src/startup.rs — `classify_bootstrap_action(Option<&InstalledAppRecord>)` (record-only); the `genesis_bootstrap_app_runtime_ready` probe removed; unit test `classify_bootstrap_keeps_installed_genesis_regardless_of_hash`. 36/37 startup tests green.
