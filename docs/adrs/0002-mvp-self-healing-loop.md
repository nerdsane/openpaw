...
The daemon reads local credentials from a gitignored `.env` file via `dotenv`. Supported MVP secrets include Anthropic, Discord, GitHub, Datadog, and sandbox-provider credentials. Startup seeds those values into the Temper secrets vault for both the default tenant and the active tenant.
