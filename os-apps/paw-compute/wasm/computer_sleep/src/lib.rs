//! computer_sleep — suspend the Computer's Tensorlake sandbox (ARN-466).
//!
//! Fires on Sleep (Ready → Sleeping). POSTs /sandboxes/{id}/suspend. Not a
//! best-effort no-op: an empty machine_id or a provider error fails, and
//! SleepFailed returns the row to Ready so Temper does not claim Sleeping
//! while the box is still billing.
//!
...
pub extern "C" fn run(_ctx_ptr: i32, _ctx_len: i32) -> i32 {
    let result = (|| -> Result<(), String> {
        ...
        if machine_id.is_empty() {
            return Err("computer_sleep: no machine_id — cannot suspend".to_string());
        }

        sandbox::sandbox_suspend(&ctx, &provider, &machine_id)?;
        ctx.log("info", &format!("computer_sleep: suspended {machine_id}"));
