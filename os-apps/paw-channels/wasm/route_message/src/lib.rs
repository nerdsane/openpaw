    ...
    let result = (|| -> Result<(), String> {
        ...
        let agent_id = if let Some(session) = existing_session {
            ...
            if steer_ok {
                ...
                let _ = ctx.http_call(
                    "POST",
                    &format!("{temper_api_url}/tdata/ChannelSessions('{session_id}')/OpenPaw.ChannelSession.Expire"),
                    &odata_headers(&ctx.tenant),
                    "{}",
                );

                // Query the previous agent's state for session continuity
                let prev_agent = if !agent_id.is_empty() {
                    get_agent_state(&ctx, &temper_api_url, &ctx.tenant, &agent_id).ok()
                } else {
                    None
                };
                ...
                let route_config = route
                    .as_ref()
                    .and_then(|value| nested_str_field(value, &["agent_config"]).or_else(|| nested_str_field(value, &["AgentConfig"])))
                    .filter(|value| !value.trim().is_empty())
                    .unwrap_or(default_agent_config);
                let route_soul_id = route
                    .as_ref()
                    .and_then(|value| nested_str_field(value, &["soul_id"]).or_else(|| nested_str_field(value, &["SoulId"])))
                    .unwrap_or("");

                let new_agent_id = if let Some(ref prev) = prev_agent {
                    // Resume: create new agent inheriting previous agent's session context
                    resume_agent_from_previous(
                        &ctx, &temper_api_url, &ctx.tenant,
                        route_config, route_soul_id, content, prev,
                    )?
                } else {
                    create_agent_from_route(
                        &ctx, &temper_api_url, &ctx.tenant,
...
fn resume_session(
    ...
    let url = format!(
        "{temper_api_url}/tdata/ChannelSessions('{session_id}')/OpenPaw.ChannelSession.Resume"
    );
...
fn create_agent_from_route(
    ...
    let create_resp = ctx.http_call("POST", &format!("{temper_api_url}/tdata/Agents"), &odata_headers(tenant), "{}")?;
    ...
    let configure_body = json!({
        ...
        "max_turns": config.get("max_turns").and_then(Value::as_str).unwrap_or("100"),
    ...
    let configure_url = format!(
        "{temper_api_url}/tdata/Agents('{agent_id}')/OpenPaw.Agent.Configure"
    );
    ...
    if !(200..300).contains(&configure_resp.status) {
        return Err(format!("configure Agent failed (HTTP {})", configure_resp.status));
    }

    let provision_url = format!(
        "{temper_api_url}/tdata/Agents('{agent_id}')/OpenPaw.Agent.Provision"
    );
    let provision_resp = ctx.http_call("POST", &provision_url, &odata_headers(tenant), "{}")?;
    if !(200..300).contains(&provision_resp.status) {
        return Err(format!("provision Agent failed (HTTP {})", provision_resp.status));
    }
    Ok(agent_id)
}
...
fn create_session(
    ...
    let create_url = format!(
        "{temper_api_url}/tdata/ChannelSessions('{session_id}')/OpenPaw.ChannelSession.Create"
    );
    let body = json!({
...
fn steer_existing_agent(
    ...
    let steer_url = format!(
        "{temper_api_url}/tdata/Agents('{agent_id}')/OpenPaw.Agent.Steer"
    );
    let body = json!({
...
fn get_agent_state(
    ...
    let resp = ctx.http_call("GET", &url, &odata_headers(tenant), "")?;
    if resp.status != 200 {
        return Err(format!("GET Agent({agent_id}) failed (HTTP {})", resp.status));
    }
    serde_json::from_str(&resp.body).map_err(|e| format!("parse agent state: {e}"))
}

fn resume_agent_from_previous(
    ctx: &Context,
    temper_api_url: &str,
    tenant: &str,
    route_config: &str,
    ...
    // Extract session/workspace state from previous agent
    let session_file_id = nested_str_field(prev_agent, &["SessionFileId", "session_file_id"]).unwrap_or("");
    let session_leaf_id = nested_str_field(prev_agent, &["SessionLeafId", "session_leaf_id"]).unwrap_or("");
    let workspace_id = nested_str_field(prev_agent, &["WorkspaceId", "workspace_id"]).unwrap_or("");
    let sandbox_url = nested_str_field(prev_agent, &["SandboxUrl", "sandbox_url"]).unwrap_or("");
    let sandbox_id = nested_str_field(prev_agent, &["SandboxId", "sandbox_id"]).unwrap_or("");
    let file_manifest_id = nested_str_field(prev_agent, &["FileManifestId", "file_manifest_id"]).unwrap_or("");
    let conversation_file_id = nested_str_field(prev_agent, &["ConversationFileId", "conversation_file_id"]).unwrap_or("");

    // If no session context exists, fall back to fresh provisioning
    if session_file_id.is_empty() && workspace_id.is_empty() {
        return create_agent_from_route(ctx, temper_api_url, tenant, route_config, route_soul_id, user_message);
    }
    ...
    // Configure with same soul and settings
    let configure_body = json!({
        "system_prompt": config.get("system_prompt").and_then(Value::as_str).unwrap_or(""),
        "user_message": user_message,
        "model": config.get("model").and_then(Value::as_str).unwrap_or("claude-sonnet-4-20250514"),
    ...
    // Resume with previous agent's session context instead of fresh Provision
    ...
    let resume_url = format!(
        "{temper_api_url}/tdata/Agents('{agent_id}')/OpenPaw.Agent.Resume"
    );
    let resume_resp = ctx.http_call("POST", &resume_url, &odata_headers(tenant), &resume_body.to_string())?;
    if !(200..300).contains(&resume_resp.status) {
        return Err(format!("resume Agent failed (HTTP {})", resume_resp.status));
    }
