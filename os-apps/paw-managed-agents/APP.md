...
- `ManagedSession` — lifecycle wrapper around one inner `TemperPaw.Session`
...
`ManagedSession` creates and steers an inner `TemperPaw.Session`, while the
...
This app is also the first TemperPaw app to lean heavily on Temper
