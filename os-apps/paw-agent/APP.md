...
Depends on `paw-fs` for file storage (soul content, agent instructions). Install from Genesis with a pinned ref such as `temper.install_app({"app_ref":"temperpaw/paw-agent@HASH","follow_policy":"pinned"})`.
