...
anthropic_api_key (or api_key) for anthropic, openrouter_api_key (or api_key) for openrouter"
        ...
        // Workspace ID for creating content files
        ...
        let (mut messages, mut session_tree) = if use_session_tree {
            ...
            if session_jsonl.is_empty() {
                ...
                let tree = SessionTree::from_jsonl(&session_jsonl);
                // Use content-file-aware context building
        ...
        // Skip legacy conversation file write when using session tree mode —
        // the session tree with content_file_id references is the source of truth.
        ...
        match response.stop_reason.as_str() {
            "tool_use" => {
                ...
                // Update session tree if in tree mode — store content as separate file
                let new_leaf = if use_session_tree {
                    if let Some(ref mut tree) = session_tree {
                        let parent = session_leaf_id;
                        let entry_id = format!("a-{}", tree.len());
                        let content_str = serde_json::to_string(&response.content).unwrap_or_default();
                        let content_file_id = create_content_file_for_entry(
                            &ctx, &temper_api_url, tenant, workspace_id, &entry_id, &content_str,
                        )?;
            ...
            "end_turn" | "stop" => {
                ...
                if use_session_tree {
                    if let Some(ref mut tree) = session_tree {
                        ...
                        let (new_leaf, _) = tree.append_assistant_message_file(
                            parent,
                            &content_file_id,
                            response.output_tokens as usize,
                        );
...
fn simulate_mock_hang(ctx: &Context) -> Result<(), String> {
    let base_url = resolve_temper_api_url(ctx, &serde_json::Value::Null);
    let url = format!(
...
fn build_tool_definitions(tools_enabled: &str, sandbox_url: &str, workdir: &str) -> Vec<Value> {
        ...
        tools.push(json!({
            ...
            "input_schema": {
                ...
                "properties": {
                    ...
                    "service_name": { "type": "string", "description": "Optional service filter. Defaults to openpaw." },
    ...
    // OData entity management tools
    if enabled.contains(&"temper_create") {
        tools.push(json!({
            "name": "temper_create",
            "description": "Create a new entity via the OData API. Returns the created entity with its ID.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "entity_set": { "type": "string", "description": "The entity set name (e.g. ProjectHarnesses, AlertCycles, WorkCycles, Agents, Issues)" },
                    "body": { "type": "object", "description": "Optional initial properties (usually empty {})" }
                },
                "required": ["entity_set"]
            }
        }));
    ...
    if enabled.contains(&"temper_action") {
        tools.push(json!({
            "name": "temper_action",
            "description": "Dispatch an action on an existing entity. Actions transition the entity through its state machine.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "entity_set": { "type": "string", "description": "The entity set name (e.g. ProjectHarnesses, AlertCycles)" },
                    "entity_id": { "type": "string", "description": "The entity ID to act on" },
                    "action": { "type": "string", "description": "The qualified action name (e.g. OpenPaw.ProjectHarness.Configure, OpenPaw.AlertCycle.Open)" },
                    "body": { "type": "object", "description": "Action parameters as a JSON object" }
                },
    ...
    if enabled.contains(&"temper_list") {
        tools.push(json!({
            "name": "temper_list",
            "description": "Query entities from the OData API. Returns a list of entities matching the optional filter.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "entity_set": { "type": "string", "description": "The entity set name (e.g. ProjectHarnesses, Agents, Issues)" },
                    "filter": { "type": "string", "description": "Optional OData $filter expression (e.g. Status eq 'Active')" }
                },
                "required": ["entity_set"]
            }
        }));
...
/// Resolve context refs into LLM messages by fetching content from TemperFS Files.
    ...
    for ctx_ref in refs {
        match ctx_ref.entry_type {
            EntryType::Compaction => {
                // Fetch compaction summary from content file, or use inline fallback
            ...
            EntryType::Message | EntryType::Steering => {
                if let Some(ref file_id) = ctx_ref.content_file_id {
                    // Fetch content from TemperFS File
                    let raw = read_content_file_raw(ctx, temper_api_url, tenant, file_id)?;
                    if raw.is_empty() {
                        continue;
                    }
                    // Content may be JSON (for assistant content blocks) or plain text
                ...
                // If no content_file_id, skip (no inline content in new architecture)
            }
            EntryType::Header => {
                // Skip headers
            }
...
/// Read raw content from a TemperFS File by ID.
...
/// Create a TemperFS File for a session entry's content.
...
fn create_content_file_for_entry(
    ...
    ctx.log("info", &format!("llm_caller: created content file {file_id} for entry {entry_id} ({} bytes)", content.len()));
    Ok(file_id)
}
