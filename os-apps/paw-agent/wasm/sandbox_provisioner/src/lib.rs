...
fn create_session_tree(
    ...
    let content_file_resp = match ctx.http_call(
        "POST",
        &format!("{temper_api_url}/tdata/Files"),
        ...
        Ok(resp) => resp,
        Err(e) => {
            ctx.log("warn", &format!("Failed to create content file for first user msg: {e}"));
            return (String::new(), String::new());
        }
    };

    let content_file_id = if content_file_resp.status >= 200 && content_file_resp.status < 300 {
        let parsed: Value = serde_json::from_str(&content_file_resp.body).unwrap_or(json!({}));
        parsed.get("entity_id").and_then(|v| v.as_str()).unwrap_or("").to_string()
    } else {
        ctx.log("warn", &format!("Content file creation failed (HTTP {})", content_file_resp.status));
        return (String::new(), String::new());
    };

    if !content_file_id.is_empty() {
        // Write the user message content to the file
        let content_write_url = format!("{temper_api_url}/tdata/Files('{content_file_id}')/$value");
        ...
        match ctx.http_call("PUT", &content_write_url, &content_write_headers, user_message) {
            Ok(resp) if resp.status >= 200 && resp.status < 300 => {
                ctx.log("info", &format!("sandbox_provisioner: wrote user message content to file {content_file_id}"));
            }
            Ok(resp) => {
                ctx.log("warn", &format!("Content file write failed (HTTP {})", resp.status));
            }
            Err(e) => {
                ctx.log("warn", &format!("Content file write error: {e}"));
            }
        }
    }

    // Initialize session file with JSONL header + first user message (reference only)
