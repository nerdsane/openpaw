...
use wasm_helpers::{create_content_file, read_session_from_temperfs, resolve_temper_api_url, write_session_to_temperfs};
...
pub extern "C" fn run(_ctx_ptr: i32, _ctx_len: i32) -> i32 {
    let result = (|| -> Result<(), String> {
        ...
        // Use content-file-aware refs to resolve content from TemperFS
        let context_refs = tree.build_context_refs(&cut_point);
        let messages_to_summarize = resolve_context_refs_for_compaction(&ctx, &temper_api_url, tenant, &context_refs);
        ...
        // 5. Store summary as a TemperFS File and append compaction entry
        let (compaction_id, _line) = if !workspace_id.is_empty() {
            match create_content_file(&ctx, &temper_api_url, tenant, workspace_id, &format!("compaction-{}.txt", tree.len()), &summary) {
                Ok(summary_file_id) => {
                    ctx.log("info", &format!("context_compactor: created summary file {summary_file_id}"));
                    tree.append_compaction_file(session_leaf_id, &summary_file_id, &cut_point)
                }
                Err(e) => {
                    ctx.log("warn", &format!("context_compactor: failed to create summary file: {e}, using inline"));
...
/// Resolve context refs into messages for compaction summarization.
/// Fetches content from TemperFS Files where content_file_id is set.
        ...
        match ctx_ref.entry_type {
            session_tree_lib::EntryType::Compaction => {
                let summary = if let Some(ref file_id) = ctx_ref.content_file_id {
                    wasm_helpers::read_content_file(ctx, temper_api_url, tenant, file_id)
            ...
            session_tree_lib::EntryType::Message | session_tree_lib::EntryType::Steering => {
                if let Some(ref file_id) = ctx_ref.content_file_id {
                    match wasm_helpers::read_content_file(ctx, temper_api_url, tenant, file_id) {
