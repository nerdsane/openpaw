...
/// Query Datadog via their REST API.
///
/// Supported query kinds:
/// - `monitor`: Get monitor details by ID (`monitor_id` param)
/// - `monitors`: List/search monitors (`query` or `tags` param)
/// - `events`: Query recent events (`query` param, optional `start`/`end` epoch)
/// - `metrics`: Query metrics timeseries (`query` param, `from`/`to` epoch)
fn query_datadog(ctx: &Context, input: &Value) -> Result<String, String> {
    ...
    let dd_app_key = ctx
        ...
        .cloned()
        .unwrap_or_default();
    if dd_api_key.trim().is_empty() || dd_api_key.contains("{secret:") {
        return Err("datadog_query: missing DD_API_KEY; configure dd_api_key secret".to_string());
    }
    if dd_app_key.trim().is_empty() || dd_app_key.contains("{secret:") {
        return Err("datadog_query: missing DD_APP_KEY; configure dd_app_key secret".to_string());
    }
    ...
    let base_url = format!("https://api.{dd_site}");
    ...
    ctx.log(
        "info",
        &format!("tool_runner: querying Datadog, query_kind={query_kind}"),
    );
    ...
    match query_kind {
        "monitor" => {
            let monitor_id = input
                .get("monitor_id")
                .and_then(Value::as_str)
                .ok_or("datadog_query: 'monitor_id' required for query_kind=monitor")?;
            let url = format!("{base_url}/api/v1/monitor/{monitor_id}");
            let resp = ctx.http_call("GET", &url, &headers, "")?;
            if resp.status < 200 || resp.status >= 300 {
                return Err(format!(
                    "datadog_query monitor failed (HTTP {}): {}",
                    resp.status,
                    truncate_tool_output(&resp.body, 1200)
                ));
            }
            Ok(truncate_tool_output(&resp.body, 6_000))
        }
        "monitors" => {
            let query = input
                .get("query")
            ...
            let tags = input
                ...
                .and_then(Value::as_str)
                .unwrap_or("");
            let mut url = format!("{base_url}/api/v1/monitor?");
            if !query.is_empty() {
                url.push_str(&format!("query={}&", url_encode(query)));
            }
            if !tags.is_empty() {
                url.push_str(&format!("monitor_tags={}&", url_encode(tags)));
            }
            let resp = ctx.http_call("GET", &url, &headers, "")?;
            if resp.status < 200 || resp.status >= 300 {
                return Err(format!(
                    "datadog_query monitors failed (HTTP {}): {}",
                    resp.status,
                    truncate_tool_output(&resp.body, 1200)
                ));
            }
        ...
        "events" => {
            ...
            let start = input
                ...
                .unwrap_or_else(|| {
                    // Default: 1 hour ago (epoch seconds)
                    let now_approx = 1711756800_i64; // placeholder; agents pass real values
                    now_approx - 3600
                });
            let end = input
                .get("end")
            ...
            let url = format!(
                "{base_url}/api/v1/events?start={start}&end={end}&sources={}",
                url_encode(query)
            );
            let resp = ctx.http_call("GET", &url, &headers, "")?;
            if resp.status < 200 || resp.status >= 300 {
                return Err(format!(
                    "datadog_query events failed (HTTP {}): {}",
                    resp.status,
                    truncate_tool_output(&resp.body, 1200)
                ));
            }
        ...
        "metrics" => {
            let query = input
                .get("query")
                .and_then(Value::as_str)
                .ok_or("datadog_query: 'query' required for query_kind=metrics")?;
            let from = input
                .get("from")
                .and_then(Value::as_i64)
                .unwrap_or(0);
            ...
            if resp.status < 200 || resp.status >= 300 {
                return Err(format!(
                    "datadog_query metrics failed (HTTP {}): {}",
                    resp.status,
                    truncate_tool_output(&resp.body, 1200)
                ));
            }
        ...
        other => Err(format!(
            "datadog_query: unsupported query_kind '{other}'; use monitor, monitors, events, or metrics"
        )),
    }
}
