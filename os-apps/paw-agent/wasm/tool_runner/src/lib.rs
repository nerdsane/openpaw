    ...
    let result = (|| -> Result<(), String> {
        ...
        for call in &tool_calls {
                ...
                evaluate_before_hooks(&ctx, &temper_api_url, tenant, soul_id, hook_policy, tool_name)?
            {
                ...
                execute_tool(&ctx, sandbox_url, workdir, tool_name, &input, &temper_api_url, &ctx.tenant)
            };
        ...
        if !session_file_id.is_empty() && !session_leaf_id.is_empty() {
            // Session tree mode: store tool results as separate content file
            ...
            // Create a TemperFS File for the tool results content
            let workspace_id = fields
                .get("workspace_id")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let entry_id = format!("t-{}", tree.len());
            let content_str = serde_json::to_string(&tool_results_value).unwrap_or_default();

            let content_file_id = if !workspace_id.is_empty() {
                create_tool_content_file(&ctx, &temper_api_url, tenant, workspace_id, &entry_id, &content_str)?
            } else {
                // Fallback: if no workspace_id, use inline (shouldn't happen in practice)
                String::new()
            };

            let (new_leaf, _) = if !content_file_id.is_empty() {
        ...
        // Only fsync for E2B sandboxes (ephemeral — files need to persist in TemperFS).
        // Local sandboxes persist files on disk, no sync needed.
        let e2b = is_e2b_sandbox(sandbox_url);
        if e2b && !file_manifest_id.is_empty() && !workspace_id.is_empty() && !sandbox_url.is_empty() {
...
/// Check if a bash command needs git credentials (push, clone with https, fetch with auth, etc.)
fn needs_git_credentials(command: &str) -> bool {
    let cmd = command.to_lowercase();
    cmd.contains("git push")
        || cmd.contains("git clone")
        || cmd.contains("git fetch")
        || cmd.contains("git pull")
        || cmd.contains("gh pr create")
...
fn execute_tool(
    ...
    match tool_name {
        ...
        "temper_create" => temper_create(ctx, temper_api_url, tenant, input),
        "temper_action" => temper_action(ctx, temper_api_url, tenant, input),
        "temper_list" => temper_list(ctx, temper_api_url, tenant, input),
        unknown => Err(format!("unknown tool: {unknown}")),
    }
}

fn temper_create(ctx: &Context, api_url: &str, tenant: &str, input: &Value) -> Result<String, String> {
    let entity_set = input.get("entity_set").and_then(|v| v.as_str())
    ...
    if !(200..300).contains(&resp.status) {
        return Err(format!("temper_create {entity_set} failed (HTTP {}): {}", resp.status, &resp.body[..resp.body.len().min(200)]));
    }
    Ok(resp.body)
}

fn temper_action(ctx: &Context, api_url: &str, tenant: &str, input: &Value) -> Result<String, String> {
    let entity_set = input.get("entity_set").and_then(|v| v.as_str())
    ...
    let body = input.get("body").cloned().unwrap_or(json!({}));
    let url = format!("{api_url}/tdata/{entity_set}('{entity_id}')/{action}");
    let headers = odata_headers(tenant);
    let body_str = serde_json::to_string(&body).map_err(|e| format!("JSON error: {e}"))?;
    let resp = ctx.http_call("POST", &url, &headers, &body_str)?;
    if !(200..300).contains(&resp.status) {
        return Err(format!("temper_action {action} failed (HTTP {}): {}", resp.status, &resp.body[..resp.body.len().min(200)]));
    }
    Ok(resp.body)
}

fn temper_list(ctx: &Context, api_url: &str, tenant: &str, input: &Value) -> Result<String, String> {
    let entity_set = input.get("entity_set").and_then(|v| v.as_str())
        .ok_or("temper_list: missing 'entity_set'")?;
    let filter = input.get("filter").and_then(|v| v.as_str()).unwrap_or("");
    let url = if filter.is_empty() {
        format!("{api_url}/tdata/{entity_set}")
    } else {
        format!("{api_url}/tdata/{entity_set}?$filter={filter}")
    };
    let headers = odata_headers(tenant);
    let resp = ctx.http_call("GET", &url, &headers, "")?;
    if !(200..300).contains(&resp.status) {
        return Err(format!("temper_list {entity_set} failed (HTTP {}): {}", resp.status, &resp.body[..resp.body.len().min(200)]));
    }
    Ok(resp.body)
}
...
fn run_bash_local(
    ...
    // If the command involves git operations that need auth, inject GITHUB_TOKEN
    let effective_command = if needs_git_credentials(command) {
        if let Some(token) = ctx.config.get("github_token").filter(|t| !t.trim().is_empty()) {
            ctx.log("info", "tool_runner: injecting git credentials for authenticated git operation");
            format!(
...
fn send_heartbeat(ctx: &Context, temper_api_url: &str, tenant: &str) -> Result<(), String> {
    let url = format!(
        "{temper_api_url}/tdata/Agents('{}')/OpenPaw.Agent.Heartbeat",
        ctx.entity_id
    );
...
fn load_matching_hooks(
    ...
    let url = format!("{temper_api_url}/tdata/Hooks");
    ...
    match tool_name {
            ...
            if resp.status >= 200 && resp.status < 300 {
                ...
                if !entity_id.is_empty() {
                    let action_url = format!(
                        "{temper_api_url}/tdata/Memories('{entity_id}')/OpenPaw.Memory.Save"
                    );
        ...
        "spawn_agent" => {
            ...
            let create_body = if requested_id.is_empty() {
                ...
                json!({ "AgentId": requested_id }).to_string()
            };
            ...
            let config_url = format!(
                "{temper_api_url}/tdata/Agents('{child_id}')/OpenPaw.Agent.Configure"
            );
            ...
            // 3. Provision
            let prov_url = format!(
                "{temper_api_url}/tdata/Agents('{child_id}')/OpenPaw.Agent.Provision"
            );
        ...
        "list_agents" => {
            let parent_id = ctx.entity_state.get("entity_id").and_then(|v| v.as_str()).unwrap_or("");
            let agents = list_agents(ctx, temper_api_url, tenant)?;
        ...
        "abort_agent" => {
            ...
            let url = format!(
                "{temper_api_url}/tdata/Agents('{resolved_agent_id}')/OpenPaw.Agent.Cancel"
            );
        ...
        "steer_agent" => {
            ...
            let url = format!(
                "{temper_api_url}/tdata/Agents('{resolved_agent_id}')/OpenPaw.Agent.Steer"
            );
...
fn list_agents(
    ...
    let resp = ctx.http_call("GET", &url, &odata_headers(tenant), "")?;
    if resp.status != 200 {
        return Err(format!("agent listing failed (HTTP {})", resp.status));
    }
...
fn resolve_agent_reference(
    ...
    Ok(agents.into_iter().find(|agent| {
        ...
        let agent_id_field = entity_field_str(agent, &["AgentId"]).unwrap_or("");
        entity_id == agent_reference || agent_id_field == agent_reference
    }))
}
...
/// Create a TemperFS File for tool result content. Returns the file entity ID.
fn create_tool_content_file(
    ...
    ctx.log("info", &format!("tool_runner: created content file {file_id} for entry {entry_id} ({} bytes)", content.len()));
    Ok(file_id)
}
