...
pub struct SessionEntry {
    ...
    pub data: Value,
    pub tokens: usize,
    /// TemperFS File entity ID containing the actual content.
    /// When set, content is stored externally rather than inline.
...
/// Content must be fetched from TemperFS using `content_file_id`.
#[derive(Debug, Clone)]
pub struct ContextRef {
    ...
    pub content_file_id: Option<String>,
    pub entry_type: EntryType,
    /// For compaction entries, the inline summary (fallback if no content_file_id).
...
impl SessionTree {
    ...
    pub fn from_jsonl(data: &str) -> Self {
        ...
        for line in data.lines() {
            ...
            if let Ok(val) = serde_json::from_str::<Value>(line) {
                ...
                if !id.is_empty() {
                    let content_file_id = val.get("content_file_id")
                        .and_then(|v| v.as_str())
                        .filter(|s| !s.is_empty())
                        .map(|s| s.to_string());
    ...
    ///
    /// NOTE: This method only works with inline content (legacy mode).
    /// For the content-file architecture, use `build_context_refs` instead
    /// and resolve content_file_ids externally.
    ...
    /// Build context entry references by walking from leaf_id to root.
    /// Returns a list of (entry_id, role, content_file_id_or_none, entry_type) tuples
    /// in root-to-leaf order. Callers must fetch content from TemperFS for each
    /// entry that has a `content_file_id`.
    ///
    /// When a compaction entry is encountered, all entries before it are dropped
    /// (the compaction's content_file_id contains the summary).
            ...
            match entry.entry_type {
                ...
                EntryType::Compaction => {
                    ...
                    refs.push(ContextRef {
                        ...
                        inline_summary: entry.data.get("summary").and_then(|v| v.as_str()).map(|s| s.to_string()),
                    });
                }
                EntryType::Message | EntryType::Steering => {
                    let role = entry.data.get("role").and_then(|v| v.as_str()).unwrap_or("user").to_string();
                    refs.push(ContextRef {
                        entry_id: entry.id.clone(),
                        role,
                        content_file_id: entry.content_file_id.clone(),
    ...
    /// If `content_file_id` is provided, only the file reference is stored (no inline content).
    pub fn append_entry(
        &mut self,
        id: &str,
        ...
        let cfile_id = data.get("content_file_id")
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
    ...
    /// No inline content is stored — only the `content_file_id` reference.
    /// Returns the JSONL line for the new entry.
    ...
    /// Append a user message with inline content. Returns (entry_id, jsonl_line).
    ...
    /// Append a user message with content stored in a TemperFS File. Returns (entry_id, jsonl_line).
    pub fn append_user_message_file(&mut self, parent_id: &str, content_file_id: &str, tokens: usize) -> (String, String) {
        let id = format!("u-{}", self.order.len());
    ...
    /// Append an assistant message with inline content. Returns (entry_id, jsonl_line).
    ...
    /// Append an assistant message with content stored in a TemperFS File. Returns (entry_id, jsonl_line).
    pub fn append_assistant_message_file(&mut self, parent_id: &str, content_file_id: &str, tokens: usize) -> (String, String) {
        let id = format!("a-{}", self.order.len());
        let line = self.append_entry_with_file(
            &id,
            Some(parent_id),
    ...
    /// Append a tool result message with inline content. Returns (entry_id, jsonl_line).
    ...
    /// Append a tool result message with content stored in a TemperFS File. Returns (entry_id, jsonl_line).
    pub fn append_tool_results_file(&mut self, parent_id: &str, content_file_id: &str, tokens: usize) -> (String, String) {
        let id = format!("t-{}", self.order.len());
        let line = self.append_entry_with_file(
            &id,
            Some(parent_id),
    ...
    /// Append a compaction entry with inline summary. Returns (entry_id, jsonl_line).
    ...
    /// Append a compaction entry with summary stored in a TemperFS File. Returns (entry_id, jsonl_line).
    pub fn append_compaction_file(&mut self, parent_id: &str, content_file_id: &str, first_kept: &str) -> (String, String) {
        let id = format!("c-{}", self.order.len());
        let extra = json!({
            "first_kept": first_kept,
        });
    ...
    /// Append a steering message with inline content. Returns (entry_id, jsonl_line).
