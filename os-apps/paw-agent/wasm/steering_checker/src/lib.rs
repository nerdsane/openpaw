...
pub extern "C" fn run(_ctx_ptr: i32, _ctx_len: i32) -> i32 {
    let result = (|| -> Result<(), String> {
        ...
        if !steering_messages.is_empty() && follow_up_count < max_follow_ups {
            ...
            // If session tree mode, inject into session tree with content file
            if !session_file_id.is_empty() && !session_leaf_id.is_empty() {
                ...
                // Create content file for steering message, then reference it
                let entry_id = format!("s-{}", tree.len());
                let new_leaf_id = if !workspace_id.is_empty() {
                    match create_content_file(&ctx, &temper_api_url, tenant, workspace_id, &format!("msg-{entry_id}.txt"), msg_content) {
                        Ok(content_file_id) => {
                        ...
                        Err(e) => {
                            ctx.log("warn", &format!("steering_checker: content file creation failed: {e}, using inline"));
                            let (leaf, _) = tree.append_steering_message(session_leaf_id, msg_content, estimate_tokens(msg_content));
                            leaf
                        }
                    }
                } else {
