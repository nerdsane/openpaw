...
//!
//! Mirrors `temper-sandbox/src/convert.rs` for consistent behavior.
    ...
    match value {
        ...
        MontyObject::Dict(pairs) => {
            ...
            for (key, value) in pairs {
                ...
                out.insert(key_str, monty_object_to_json(value));
            }
            Value::Object(out)
        }
        ...
        // Catch-all for any variants not handled
        _ => json!({"$unknown": format!("{value:?}")}),
    }
}
