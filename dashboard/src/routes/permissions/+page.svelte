<script lang="ts">
  ...
  import { fetchDecisions, fetchPolicies, queryEntities, getEntity, type PendingDecision, type PolicyEntry } from '$lib/api';
  import StatusBadge from '$lib/components/StatusBadge.svelte';
  import GatePipeline from '$lib/components/GatePipeline.svelte';
  import type { WorkCycle, EntityEvent } from '$lib/types';
  ...
  onMount(async () => {
    const [ag, so, ha, wc, sk, th, dec, pol] = await Promise.all([
...
<div class="page">
  <header class="page-header">
    <h1>Permissions</h1>
    <p class="subtitle">Team setup, agent permissions, harness governance, and authorization decisions</p>
  </header>
  ...
  {:else}
    ...
    <section class="section">
      ...
      <p class="section-desc">All agent instances with their assigned roles and tool permissions</p>
      {#if agents.length === 0}
        <p class="empty-text">No agents have run yet</p>
      {:else}
        <div class="list">
          {#each agents as agent (field(agent, 'Id'))}
            {@const agentId = field(agent, 'Id')}
            {@const soulId = field(agent, 'soul_id')}
            {@const tools = field(agent, 'tools_enabled')}
            {@const status = field(agent, 'Status')}
            <div class="list-row">
              <div class="list-header" onclick={() => expandedAgent = expandedAgent === agentId ? null : agentId}>
                <span class="list-dot" style:background={statusColor(status)}></span>
                <span class="list-name">{soulId || 'Agent'}</span>
                <code class="list-id">{shortId(agentId)}</code>
                <StatusBadge {status} />
              ...
              {#if expandedAgent === agentId}
                <div class="list-detail" transition:slide={{ duration: 150 }}>
                  <div class="detail-grid">
                    ...
                    <code class="detail-value">{agentId}</code>

                    <span class="detail-label">Soul / Role</span>
                    <span class="detail-value">{soulId || '--'}</span>

                    <span class="detail-label">Model</span>
                    <span class="detail-value">{field(agent, 'model')}</span>

                    <span class="detail-label">Provider</span>
                    <span class="detail-value">{field(agent, 'provider')}</span>

                    <span class="detail-label">Tools Enabled</span>
                    <div class="tool-list">
                      {#each (tools || '').split(',').filter(Boolean) as tool}
                        <code class="tool-tag">{tool.trim()}</code>
                      {/each}
                    </div>

                    <span class="detail-label">Max Turns</span>
                    <span class="detail-value">{field(agent, 'max_turns') || '--'}</span>

                    <span class="detail-label">Turns Used</span>
                    <span class="detail-value">{agent.turn_count ?? 0}</span>

                    <span class="detail-label">Tokens</span>
                    <span class="detail-value">in: {((agent.input_tokens as number) ?? 0).toLocaleString()} / out: {((agent.output_tokens as number) ?? 0).toLocaleString()}</span>

                    {#if field(agent, 'parent_agent_id')}
                      <span class="detail-label">Parent Agent</span>
                      <a href="/agents/{field(agent, 'parent_agent_id')}" class="detail-link">{shortId(field(agent, 'parent_agent_id'))}</a>
                    {/if}

                    {#if field(agent, 'project_harness_id')}
                      <span class="detail-label">Harness</span>
                      <code class="detail-value">{shortId(field(agent, 'project_harness_id'))}</code>
                    {/if}
                  </div>
                </div>
              {/if}
    ...
    {#if toolHooks.length > 0}
      <section class="section">
        <h2 class="section-title">Tool Hooks</h2>
        <p class="section-desc">Intercept tool calls to block, log, or modify behavior</p>
        <div class="list">
          {#each toolHooks as hook, i (field(hook, 'Id') || i)}
            <div class="list-row">
              <div class="list-header">
                <span class="list-dot" style:background={field(hook, 'hook_action') === 'block' ? 'var(--status-error)' : 'var(--status-warning)'}></span>
                <span class="list-name">{field(hook, 'name') || 'Unnamed'}</span>
                <code class="tool-tag">{field(hook, 'tool_pattern') || '*'}</code>
                <span class="list-meta">{field(hook, 'hook_action')} / {field(hook, 'hook_type')}</span>
              </div>
            </div>
          {/each}
        </div>
    ...
    <section class="section">
      ...
      {#if decisions.length === 0}
        <p class="empty-inline">No authorization decisions yet — decisions appear when Cedar denies an agent action</p>
      {:else}
        <div class="list">
          {#each decisions as decision (decision.id)}
            <div class="list-row">
              <div class="list-header" onclick={() => expandedDecision = expandedDecision === decision.id ? null : decision.id}>
              ...
              {#if expandedDecision === decision.id}
                <div class="list-detail" transition:slide={{ duration: 150 }}>
                  <div class="detail-grid">
                    ...
                    <span class="detail-value" style:color="var(--status-error)">{decision.denial_reason}</span>
                    ...
                    {#if decision.generated_policy}
                      ...
                      <pre class="detail-pre">{decision.generated_policy}</pre>
                    {/if}
                  </div>
                </div>
              {/if}
    ...
    <section class="section">
      ...
      {#if policies.length === 0}
        <p class="empty-inline">No runtime Cedar policies loaded — policies come from os-app bundles and approved decisions</p>
      {:else}
        <div class="list">
...
<style>
  ...
  /* List rows (for agents, harness, decisions, policies) */
  ...
  .detail-link { font-size: var(--text-sm); color: var(--text-secondary); text-decoration: none; }
