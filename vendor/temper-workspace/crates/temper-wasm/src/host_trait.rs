...
impl WasmHost for ProductionWasmHost {
    ...
    async fn connect_call(
        ...
        let framed_body = encode_connect_request(body.as_bytes());
        ...
        if !framed_body.is_empty() {
            builder = builder.body(framed_body);
        }
...
pub fn parse_connect_frames(data: &[u8]) -> Result<Vec<String>, String> {
    ...
    while offset < data.len() {
        ...
        // Preserve trailer-frame errors so callers can surface them instead of
        // silently treating a failed stream as an empty success.
        if trailer_contains_error(&payload_str) {
            frames.push(payload_str);
        }
    }
...
fn encode_connect_request(body: &[u8]) -> Vec<u8> {
...
fn trailer_contains_error(payload: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn parse_trailer_error_payload() {
        let data = make_frame(0x02, br#"{"error":{"message":"cwd missing"}}"#);
        let frames = parse_connect_frames(&data).unwrap();
        assert_eq!(frames, vec![r#"{"error":{"message":"cwd missing"}}"#]);
    }
