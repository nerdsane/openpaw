...
pub fn parse_connect_frames(data: &[u8]) -> Result<Vec<String>, String> {
    ...
    while offset < data.len() {
        ...
        if flags & 0x02 == 0 {
            ...
            let trailer = String::from_utf8(payload.to_vec())
                .map_err(|e| format!("Connect trailer payload is not valid UTF-8: {e}"))?;
            let parsed: Value = serde_json::from_str(&trailer)
                .map_err(|e| format!("Connect trailer is not valid JSON: {e}"))?;
            if let Some(error) = parsed.get("error") {
                ...
                let message = error
                    .get("message")
                    .and_then(Value::as_str)
                    .unwrap_or("Connect stream failed");
                return Err(format!("Connect stream failed ({code}): {message}"));
            }
        }
    }
