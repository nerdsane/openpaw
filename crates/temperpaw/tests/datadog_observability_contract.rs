...
#[test]
fn temper_dependency_pin_uses_lazy_wasm_secret_authorization_revision() {
    let manifest = load_text("crates/temperpaw/Cargo.toml");
    let lockfile = load_text("Cargo.lock");
    let expected_rev = "e45e8396884e1b88c0aec7711df1eeb2d858cab0";
    ...
    for temper_crate in [
        ...
        assert!(
            ...
            "{temper_crate} must pin the Temper rev with lazy WASM secret authorization, WASM dispatch envelope attribution, data-only create fast path support, projection read parity, local TData tenant propagation, runtime-derived LLMObs service identity, parent stitching, agent/workflow hierarchy, DBM attribution, profiling envelope, Datadog-visible WASM span hints, host-boundary spans, guest progress/log correlation, and wasm.invoke phase tracing"
        );
    }
    ...
    assert!(
        ...
        "Cargo.lock must resolve Temper dependencies to the lazy WASM secret authorization revision"
    );
}
...
fn wasm_guest_observability_live_proof_is_temper_native_and_datadog_backed() {
    ...
    assert!(
        probe_manifest.contains("temper-wasm-sdk")
            && probe_manifest.contains("e45e8396884e1b88c0aec7711df1eeb2d858cab0"),
