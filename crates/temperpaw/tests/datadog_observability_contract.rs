...
#[test]
fn temper_dependency_pin_uses_wasm_dispatch_envelope_attribution_revision() {
    let manifest = load_text("crates/temperpaw/Cargo.toml");
    let lockfile = load_text("Cargo.lock");
    let expected_rev = "7bfb84ec1351715e4a3f961ba584a995d8792796";
    ...
    for temper_crate in [
        ...
        assert!(
            ...
            "{temper_crate} must pin the Temper rev with WASM dispatch envelope attribution, data-only create fast path support, projection read parity, local TData tenant propagation, runtime-derived LLMObs service identity, parent stitching, agent/workflow hierarchy, DBM attribution, profiling envelope, Datadog-visible WASM span hints, host-boundary spans, guest progress/log correlation, and wasm.invoke phase tracing"
        );
    }
    ...
    assert!(
        ...
        "Cargo.lock must resolve Temper dependencies to the WASM dispatch envelope attribution revision"
    );
}
...
fn wasm_guest_observability_live_proof_is_temper_native_and_datadog_backed() {
    ...
    assert!(
        probe_manifest.contains("temper-wasm-sdk")
            && probe_manifest.contains("7bfb84ec1351715e4a3f961ba584a995d8792796"),
