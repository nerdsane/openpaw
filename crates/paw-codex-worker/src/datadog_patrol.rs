...
async fn run_datadog_patrol(
    ...
    let visual_summary_url =
        datadog_visual_summary_url(active_monitors.len(), finding_ids.len(), work_cycle_ids.len());
    post_entity_action(
        client,
        config,
        "ProofPackets",
        ...
        json!({
            ...
            "changed_files_map": "No repository files changed by Datadog Patrol. Follow-up WorkCycles are approval-gated before code/deploy mutation.",
            "reviewer_verdict": "Patrol evidence packet generated by mac-mini-codex-prod; follow-up implementation WorkCycles require independent review before completion.",
            "residual_risks": "Datadog evidence indicates active monitor states; production-impacting remediation remains approval-gated in the created WorkCycles.",
        }),
    )
    .await?;
    post_entity_action(
...
struct DatadogProofSummaryInput<'a> {
    ...
    codex_analysis: &'a str,
}

fn datadog_proof_summary_markdown(input: &DatadogProofSummaryInput<'_>) -> String {
    format!(
        "# Datadog Patrol Proof\n\nPatrolRun `{patrol_run_id}` was executed by WorkerRun `{worker_run_id}`.\n\n```mermaid\n{}\n```\n\n## Result\n\n- Active Datadog monitors: {active_monitor_count}\n- Signals created: {}\n- ObservabilityFindings created: {}\n- FactoryCases created: {}\n- WorkCycles created: {}\n\n## Agent triage\n\n{}\n\n## Gate posture\n\nFollow-up WorkCycles are approval-gated before code or deploy changes. This ProofPacket is factual evidence for the patrol run, not approval to mutate production.",
        datadog_state_diagram_mermaid(),
        input.signal_ids.len(),
        input.finding_ids.len(),
        input.case_ids.len(),
        input.work_cycle_ids.len(),
        input.codex_analysis.trim(),
        patrol_run_id = input.patrol_run_id,
        worker_run_id = input.worker_run_id,
        active_monitor_count = input.active_monitor_count,
    )
}

fn datadog_state_diagram_mermaid() -> &'static str {
    "flowchart LR\n  Run[\"PatrolRun datadog_observability\"] --> Worker[\"mac-mini-codex-prod WorkerRun\"]\n  Worker --> DD[\"Read-only Datadog monitor search\"]\n  DD --> Signals[\"Signals\"]\n  DD --> Findings[\"ObservabilityFindings\"]\n  Findings --> Cases[\"FactoryCases\"]\n  Cases --> Work[\"Approval-gated WorkCycles\"]\n  Worker --> Proof[\"Ready ProofPacket\"]\n  Proof --> Complete[\"PatrolRun Complete\"]"
}
...
fn datadog_visual_summary_url(
    ...
    let svg = format!(
        "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"960\" height=\"540\" viewBox=\"0 0 960 540\" role=\"img\" aria-labelledby=\"title desc\"><title id=\"title\">Datadog Patrol proof</title><desc id=\"desc\">Factual proof summary generated from Datadog Patrol evidence.</desc><rect width=\"960\" height=\"540\" fill=\"#f7f5ef\"/><rect x=\"40\" y=\"36\" width=\"880\" height=\"468\" rx=\"8\" fill=\"#ffffff\" stroke=\"#d5d2c6\"/><text x=\"70\" y=\"92\" font-family=\"ui-sans-serif, system-ui\" font-size=\"32\" font-weight=\"700\" fill=\"#202124\">Datadog Patrol Proof</text><text x=\"70\" y=\"130\" font-family=\"ui-sans-serif, system-ui\" font-size=\"16\" fill=\"#64615a\">Read-only Datadog evidence collected by the Mac mini Codex worker.</text><rect x=\"70\" y=\"172\" width=\"230\" height=\"126\" rx=\"8\" fill=\"#fff4e5\" stroke=\"#f4c27a\"/><text x=\"96\" y=\"218\" font-family=\"ui-sans-serif, system-ui\" font-size=\"15\" fill=\"#64615a\">Active monitors</text><text x=\"96\" y=\"266\" font-family=\"ui-sans-serif, system-ui\" font-size=\"42\" font-weight=\"700\" fill=\"#a15c00\">{active_monitor_count}</text><rect x=\"330\" y=\"172\" width=\"230\" height=\"126\" rx=\"8\" fill=\"#eef5ff\" stroke=\"#b8c7dc\"/><text x=\"356\" y=\"218\" font-family=\"ui-sans-serif, system-ui\" font-size=\"15\" fill=\"#64615a\">Findings opened</text><text x=\"356\" y=\"266\" font-family=\"ui-sans-serif, system-ui\" font-size=\"42\" font-weight=\"700\" fill=\"#174ea6\">{finding_count}</text><rect x=\"590\" y=\"172\" width=\"230\" height=\"126\" rx=\"8\" fill=\"#edf4ee\" stroke=\"#b7d3bc\"/><text x=\"616\" y=\"218\" font-family=\"ui-sans-serif, system-ui\" font-size=\"15\" fill=\"#64615a\">WorkCycles gated</text><text x=\"616\" y=\"266\" font-family=\"ui-sans-serif, system-ui\" font-size=\"42\" font-weight=\"700\" fill=\"#137333\">{work_cycle_count}</text><text x=\"70\" y=\"360\" font-family=\"ui-sans-serif, system-ui\" font-size=\"18\" font-weight=\"700\" fill=\"#202124\">Flow</text><text x=\"70\" y=\"394\" font-family=\"ui-sans-serif, system-ui\" font-size=\"16\" fill=\"#202124\">PatrolRun -> Datadog evidence -> Signals -> Findings -> Cases -> approval-gated WorkCycles -> ProofPacket.</text><text x=\"70\" y=\"450\" font-family=\"ui-sans-serif, system-ui\" font-size=\"14\" fill=\"#64615a\">Production-impacting fixes remain blocked until explicit approval.</text></svg>"
    );
...
async fn analyze_datadog_evidence_with_codex(
    ...
    if !output.status.success() {
        bail!(
            "Datadog Patrol Codex analysis failed with status {:?}: {} stderr omitted from stored evidence",
            output.status.code(),
    ...
    if analysis.is_empty() {
        Ok(format!(
            "Codex analysis completed with no stdout. Stderr had {} byte(s) and was omitted from stored Patrol evidence.",
            stderr.len()
        ))
    } else {
        Ok(truncate_middle(analysis, 4_000))
    }
}
