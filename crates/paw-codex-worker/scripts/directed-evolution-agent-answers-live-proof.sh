...
write_precheck_ready_summary_and_exit() {
  ...
  summary_json="$(jq -n \
    ...
    --arg production_datadog_query_secrets_confirmed "${CONFIRM_PRODUCTION_DATADOG_QUERY_SECRETS:-0}" \
    '{
      status: $status,
  ...
  cat >"$PROOF_MD" <<EOF
# Agent Answers Directed Evolution Live Proof Precheck Ready

This file is generated by \`directed-evolution-agent-answers-live-proof.sh\`.
\`PRECHECK_ONLY=1\` validated all locally checkable operator confirmation,
control-plane identifier, worker mode, Datadog observer/evaluator evidence,
and terminal-success gates. The script did not build the worker, read metadata,
create a live episode, start a local worker, poll episode entities, or query
Datadog.

The next non-precheck run with the same environment would create a fresh live
Directed Evolution Episode for the Agent Answers organism.
...
EOF
  ...
  printf '%s\n' "$summary_json"
  log "precheck ready; proof bundle: ${PROOF_DIR}"
  exit 0
}
...
jq -n \
  ...
  '{
    ...
    human_notes: "Fresh live Agent Answers Directed Evolution proof. Requires worker provenance, simulated-user evidence, state verification, and Datadog observer/evaluator evidence before proof closure.",
